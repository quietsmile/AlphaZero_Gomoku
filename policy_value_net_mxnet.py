...
class PolicyValueNet():
    ...
    def create_policy_value_train(self, batch_size):
        ...
        #pv_train.save_params('current_policy.caffemodel') 
        #os.system('echo 1 > model.log')
    ...
    def create_policy_value_predict(self, batch_size): 
        ...
        #print('input_states', input_states_shape)
        pv_predict.bind(data_shapes=[('input_states', input_states_shape)], for_training=False)


        #pv_predict.load_params("current_policy.caffemodel")
        args, auxs = self.train_batch.get_params()
        pv_predict.set_params(args, auxs)
        
        return pv_predict
    
    def update_predict(self, fn):
        model_dict = mx.nd.load(fn)

        arg_params = {}
        aux_params = {}
        for k, value in model_dict.items():
        ...
        self.predict_one.set_params(arg_params, aux_params, allow_missing=True, allow_extra=True)    
    ...
    def predict_board(self, board_state):
        ...
        #print('state_nd', state_nd.shape)
        self.predict_one.forward(mx.io.DataBatch([state_nd]))
        acts_probs, values = self.predict_one.get_outputs()
        acts_probs = acts_probs.asnumpy()
        values = values.asnumpy()
        #print('acts_probs', acts_probs.shape)
        #print('values_probs', values.shape)
    ...
    def train_step(self, state_batch, mcts_probs, winner_batch, learning_rate):
        ...
        #self.train_batch.save_params('current_policy.caffemodel') 
        #os.system('echo 1 > model.log')
        args, auxs = self.train_batch.get_params()
...
class PolicyValueNetTrain():
    """policy-value network """
    def __init__(self, board_width, board_height, model_params=None):
        self.train_context = [mx.gpu(0)]
        #self.predict_context = [mx.gpu(i) for i in range(1,8)]
        self.batchsize = 512 #must same to the TrainPipeline's self.batch_size.
        self.predict_batchsize = 512 #must same to the TrainPipeline's self.batch_size.
        self.channelnum = 5
        self.board_width = board_width
...
class PolicyValueNetPredict():
    """policy-value network """
    def __init__(self, board_width, board_height, model_params=None):
        #self.train_context = [mx.gpu(0)]
        self.predict_context = [mx.gpu(i) for i in range(1,8)]
        self.predict_context_1 = [mx.gpu(1)]
        self.batchsize = 512 #must same to the TrainPipeline's self.batch_size.
        #self.predict_batchsize = 28 #must same to the TrainPipeline's self.batch_size.
        self.channelnum = 5
        self.board_width = board_width
        self.board_height = board_height 
        self.l2_const = 1e-4  # coef of l2 penalty 
        #self.train_batch = self.create_policy_value_train(self.batchsize)   
        #self.predict_batch = self.create_policy_value_predict(self.predict_batchsize)   
        self.predict_one = self.create_policy_value_predict(28)   
        self.num = 0
