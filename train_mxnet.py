...
#from mcts_alphaZero_virtual_loss import MCTSPlayer
    ...
    def run(self):
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                print('collect_self_play time', end - start)
                print("batch i:{}, episode_len:{}, buffer_len:{}".format(
                        i+1, self.episode_len, len(self.data_buffer)))
                print('self.data_buffer', type(self.data_buffer), len(self.data_buffer), type(self.data_buffer[0]), len(self.data_buffer[0]))
                if len(self.data_buffer) > self.batch_size:
                    loss, entropy = self.policy_update()
                    #if i % 100 == 0:
                        # this will make it worse why?
                        #self.data_buffer=[]
                # check the performance of the current model,
                # and save the model params
                if (i+1) % 50 == 0:
                    self.policy_value_net.save_model('./current_policy_%d.model' % i)
                if (i+1) % self.check_freq == 0:
                    ...
                    if win_ratio > self.best_win_ratio:
                        ...
                        if (self.best_win_ratio == 1.0 and self.pure_mcts_playout_num < 20000):
                            self.pure_mcts_playout_num += 1000
